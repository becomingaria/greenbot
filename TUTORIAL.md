...
- ✅ Persistent state for strikes + cooldowns via DynamoDB (optional, when configured).
...
### 2) Invite and configure the bot in your server

1. Create a Discord app and bot in the Discord Developer Portal.
2. Under "OAuth2 > URL Generator", choose scopes:
    - `bot`
    - `applications.commands`
    - plus permissions: `Manage Roles`, `Manage Channels`, `Ban Members`, `Kick Members`, `Send Messages`, `Read Message History`, `View Channels`, and `Moderate Members`.
3. Use the generated URL to invite bot to your guild.
4. Get your `GUILD_ID` (right-click server icon > Copy ID with Developer Mode on).
