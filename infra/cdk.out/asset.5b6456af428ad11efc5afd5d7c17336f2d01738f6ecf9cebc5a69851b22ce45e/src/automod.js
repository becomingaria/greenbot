...
function getStrikeKey(guildId, userId) {
    return `${guildId}:${userId}`
}
...
function getStrikeInfo(guildId, userId) {
    const key = getStrikeKey(guildId, userId)
    const info = strikeState.get(key)
    if (!info) return { count: 0, expiresAt: 0 }
    if (Date.now() > info.expiresAt) {
        strikeState.delete(key)
        return { count: 0, expiresAt: 0 }
    }
    return info
}

function addStrike(guildId, userId, durationMs) {
    const key = getStrikeKey(guildId, userId)
    const now = Date.now()
    const info = getStrikeInfo(guildId, userId)
    const next = {
        count: info.count + 1,
        expiresAt: now + durationMs,
    }
    strikeState.set(key, next)
    return next
}
...
async function performAction(message, action, modCfg, config, actions, reason) {
    ...
    const doBan = async (days) => {
        if (!guild) return
        await guild.members
            .ban(user.id, { days: days ?? 0, reason: `Automod${reasonText}` })
            .catch(() => null)
    }
    ...
    const doStrike = async () => {
        ...
        const strikes = addStrike(guild.id, user.id, 1000 * 60 * 60)
